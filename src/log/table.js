 ...
 */
const outputTable = (buffer, logFn = log) => {
 ...
 */
export const logTable = (tableLines, logFn = log) => {
 ...
 */
export const logLeftHeaderTable = (lines, logFn = log) => {
  const headerCells = lines.map(l => l[0])
  const dataCells = lines.map(l => l[1])

  // Get the max number of cells in any line.
  const maxDataItems = findMaxLenProp(dataCells)
  // Calculate the length of the header cells.
  const headerLength = findMaxLenFn(headerCells, stringWidth)
  // Calculate the length of each column.
  const columnLengths = range(maxDataItems).map(n => {
    const columnCells = dataCells.map(l => l[n] ? l[n] : '')
    return findMaxLenFn(columnCells, stringWidth)
  })

  const tableRows = []
  for (let a = 0; a < lines.length; ++a) {
    const row = []
    row.push({ length: headerLength, value: headerCells[a], color: assets.headerColor, isHeader: true })
    for (let b = 0; b < columnLengths.length; ++b) {
      row.push({ length: columnLengths[b], value: dataCells[a][b] ? dataCells[a][b] : '', isHeader: false })
    }
    tableRows.push(row)
  }
