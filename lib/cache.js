...
const readCache = async (cachePath, validSeconds = settings.validSeconds, parseJSON = true, isSimple = false, doLogging = false) => {
  ...
  doLogging && (0, _utils.logDebug)('Reading cache from file', isSimple ? '(no metadata)' : null, '- valid seconds', validSeconds, '- path', path);
  const exists = await cacheFileExists(path);

  if (!exists) {
    doLogging && (0, _utils.logDebug)('Cache file does not exist');
    return {
      exists,
      isStale: null,
      path,
  ...
  if (isStale) {
    doLogging && (0, _utils.logDebug)('Cache file is stale');
    return {
      exists,
      isStale,
      path,
  ...
  doLogging && (0, _utils.logDebug)('Cache read and parsed');
  return {
    exists,
    isStale,
    path,
...
const writeCache = async (dataRaw, cachePath, toJSON = true, makeDir = true, cleanJSON = true, encoding = 'utf8', doLogging = false) => {
  ...
  doLogging && (0, _utils.logDebug)('Writing cache to file', path); // Ensure the cache dir exists. TODO: need an error handler here
  ...
  if (makeDir) {
    ...
    if (!exists) {
      doLogging && (0, _utils.logDebug)('Needed to make directory', dir);
  ...
  doLogging && (0, _utils.logDebug)('Wrote cache to file', success);
  return success;
};
